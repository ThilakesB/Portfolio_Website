# <h2>Portfolio</h2>
<p><a href="https://thilakesb.vercel.app/" target="_blank">https://thilakesb.vercel.app/</a></p>

> Portfolio website of **Thilakeswaran B** — CSE student, AI/ML enthusiast, and builder.

[![Live](https://img.shields.io/badge/Live-thilakesb.vercel.app-black?style=flat-square&logo=vercel)](https://thilakesb.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Thilakeswaran--B-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/thilakeswaran-b-50b431280/)
[![GitHub](https://img.shields.io/badge/GitHub-ThilakesB-181717?style=flat-square&logo=github)](https://github.com/ThilakesB)

---

##  About

This is my personal portfolio site — built to showcase my projects, internships, skills, and background as a Computer Science Engineering student specializing in AI/ML and Web Development.

---

##  Sections

| Section | Description |
|---|---|
| **About** | Brief intro and personal summary |
| **Projects** | Highlighted builds — AI tools, web apps, automation |
| **Experience** | Internships and work history |
| **Skills** | Languages, frameworks, and tools |
| **Education** | Academic background |
| **Contact** | Links to reach me |

---

##  Featured Projects

- **Skin Disease Analyzer** — AI + LLM based skin condition prediction (MobileNetV2 / HAM10000)
- **Load Balancer System** — Round Robin traffic distribution across servers
- **AI Examination Monitoring System** — Secure browser-locked exam environment
- **AI Resume Analyzer** — Resume scoring and improvement suggestions
- **n8n Workflow Automation** — API-connected automated pipelines
- **Billing Software** — Invoice generation and transaction management
- **Password Encryption Tool** — Encrypt/decrypt sensitive data securely

---

## Experience

- **Team Lead** — HGRD Edu Tech *(mentoring students)*
- **Cybersecurity Intern** — Rinex Technology *(vulnerability analysis)*
- **Data Visualization Intern** — Tata Group *(Power BI dashboards)*

---

##  Tech Stack (Skills)

**Languages:** Python · Java · C++ · HTML/CSS/JS

**Frameworks & Tools:** React · Flask · Power BI · Git & GitHub · n8n · MCP · Linux

---

##  Contact

- 📧 [thilakesb@gmail.com](mailto:thilakesb@gmail.com)
- 💼 [LinkedIn](https://www.linkedin.com/in/thilakeswaran-b-50b431280/)
- 🐙 [GitHub](https://github.com/ThilakesB)

---

<p align="center">Built & maintained by <strong>Thilakeswaran B</strong></p>
